...
def plot_pcolor_annotate(x, y, values, azi, z_gps, T_max,
                         quantity, xlabel, ylabel, fname):
  """
  Plotting values using the SolarPILOT azimuth and height indexation:
  """
  fig = plt.figure(figsize=(3.5, 3.5))
  ...
  for i, T in enumerate(T_max):
    #print('{}: {}'.format(i, T))
    ax.annotate(
      '{:.0f}'.format(T), xy=(azi[i], z_gps[i]),
      xycoords = 'data', ha='center', va='center',
      fontsize='x-small', #fontweight='extra bold', #color='red',
      bbox=dict(boxstyle="square,pad=0.1", lw=0, fc='white')
    )
  ax2 = ax.twiny()
  ax2.set_xlabel(r'\textsc{compass}')
  ax2.set_xticks([0., .5*np.pi, np.pi, 1.5*np.pi, 2*np.pi])
  ax2.set_xticklabels(['S', 'E', 'N', 'W', 'S'])
  # ax2.set_xlabel(r'\textsc{tube}')
  # ax2.set_xticks(azi)
  # ax2.set_xticklabels(np.arange(0, 12))
  fig.tight_layout()
  fig.savefig(fname+'.pdf')
  fig.savefig(fname+'.png', dpi=150)
  plt.close('all')
